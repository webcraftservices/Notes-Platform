...
| Phase 9 — Billing, usage ledger, security hardening, and production polish | IN PROGRESS (9.1 security hardening complete) | Some plan limits and live usage calculations exist. Phase 9.1 adds: Mammoth-generated DOCX HTML sanitization before rendering, Upstash Redis-backed distributed rate limiting (in-memory fallback preserved for local dev), a generic safe-500 path + structured server-side error logging in api-response.ts, and AI provider network/timeout failures mapped to a clean 503 (`AIProviderUnavailableError`) instead of leaking raw provider exceptions. The billing/usage ledger, row-level security, and observability work are NOT yet implemented. |
